...
export class metavisuo extends page {
    ...
    async create_metadb(dbname) {
        ...
        const structure = await exec("database", [dbname, true, false], "export_structure", []);
        //
    ...
    async get_dbnames() {
        ...
        //
        //Retrieve the names
        const dbases = await exec("database", ["information_schema"], "get_sql_data", [sql]);
        //
    ...
    async show_dbase() {
        ...
        if (this.current_db !== undefined)
            this.current_db.hook.removeChild(this.current_db.proxy);
        //
...
export class database extends schema.database {
    ...
    async save() {
        ...
        const result = await exec('questionnaire', [this.meta_dbname], 'load_common', [layouts]);
        //
        //Report the result
        myalert(result);
    }
    ...
    *collect_labels() {
    ...
    async load_x_y_positions() {
        ...
        //
        //Retrieve the data 
        const result = await exec('database', [this.meta_dbname], 'get_sql_data', [sql]);
        //
    ...
    *collect_relations(dbase) {
    ...
    entity_move(ev) {
        ...
        const group = this.proxy.querySelector('.selected');
        //
...
export class entity extends schema.entity {
    ...
    */
    ...
    select() {
        ...
        const previous = this.dbase.proxy.querySelector('.selected');
        //
